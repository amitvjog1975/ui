...
const DayAccountEoD = () => {
    ...
    const handleExpenseAddClick = () => {
        ...
        setSelectedExpense({
            expenseID: 0,
            expenseAmount: 0,
            expenseDate: dayjs(eodAccountDate).format("YYYY-MM-DD"),
            expenseReason: '',
            shopID: eodShopId,
    ...
    const updateExpense = (_postData) => {
        ...
        accountService.update_shop_expenses(_postData).then(result => {
            getShopEODData(eodShopId, dayjs(eodAccountDate).format("YYYY-MM-DD"));
            getShopExpenses(eodShopId, dayjs(eodAccountDate).format("YYYY-MM-DD"));
            setIsLoading(false);
        }).catch(err => {
            console.log(err);
        });
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Grid size={{ md: 6, lg: 8, xs: 12 }}>
                                ...
                                <Grid container spacing={1} sx={{ marginTop: 2, marginBottom: 1 }}>
                                        ...
                                        <BaseCard title="Daily expenditure" headerBGColor='#bbdefb' actions={ExpenseAddAction}>
                                            <TableContainer>
                                                <Table size="small">
                                                    ...
                                                    <TableBody>
                                                        {shopExpenseData?.slice((page - 1) * 5, page * 5).map((expense) => (
                                            ...
                                            <Pagination
                                                count={Math.ceil((shopExpenseData?.length || 0) / 5)}
                                                page={page}
                                    ...
                                    <Grid size={{ sm: 6, lg: 4, md: 6, xs: 12 }}>
                                        <BaseCard title="Advance payments" headerBGColor='#bbdefb'>
                                            <AdvanceTable data={shopAdvanceData} />
                                        </BaseCard>
                                    </Grid>
                                    <Grid size={{ sm: 6, lg: 4, md: 6, xs: 12 }}>
                                        <BaseCard title="Attendance" headerBGColor='#bbdefb' >
